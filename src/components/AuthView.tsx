...
const AuthView: React.FC<AuthViewProps> = ({ initialMode, onBack, onLogin, onSignupSuccess, onForgotPassword, setView, setAuthMode }) => {
  ...
  // make the reset password to be able to recognised if the email alredy exits
  ...
  try {
    ...
    if (!existingUsers || existingUsers.length === 0) {
      setResetEmailError('This email is not registered with us. Please sign up instead.');
      setLoading(false);
      return;
    }
...
{/* Forgot Password Modal */}
<Dialog open={showForgotPw} onOpenChange={() => { setShowForgotPw(false); setResetEmailError(''); }}>
  <DialogContent className="max-w-md rounded-3xl dark:bg-slate-900">
    <DialogHeader>
      <DialogTitle className="text-2xl font-black">Reset Password</DialogTitle>
      <DialogDescription>
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          ...
          <Input
            type="email"
            placeholder="name@example.com"
            className={`pl-10 h-12 bg-slate-50 dark:bg-slate-800 rounded-xl transition-colors ${
              resetEmailError
        ...
        {resetEmailError && (
          <p className="text-red-500 text-sm font-medium mt-1">{resetEmailError}</p>
        )}
      </div>
      <Button
        onClick={() => handleForgotPassword(resetEmail)}
