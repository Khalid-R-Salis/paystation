...
//         toast.success(t('success_login') || 'Authenticating...');
//       } else {
//         // SIGNUP logic
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        // 2. Fetch the actual profile data for Local Storage
        ...
        onLogin(userData); // This triggers the LocalStorage save in App.tsx
        ...
        toast.success(t('success_signup') || "Verification email sent!");
        onSignupSuccess(normalizedEmail);
      }
    } catch (error: any) {
      toast.error(handleAuthError(error));
